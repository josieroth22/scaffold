...
**Product**
- Landing page (index.html) with 3 sample plans (Sofia, Priya, Jake), all generated by the real pipeline with CDS-backed data (July 4, 2026). Sitewide copy pass July 7: Draper-rewritten "Is this just ChatGPT?" FAQ, honest time math everywhere (20-min form), "You can close this tab" on the generating screen (was contradicting the verified resume flow), no unbuilt-feature promises, refund path via reply-to, CTAs at three conviction moments (post-how-it-works, post-samples, post-FAQ with "Twenty minutes of your time. $50. Tonight.")
...
- Plan renderer (plan.html): sidebar navigation, collapsible sections, simulation charts, submission info; still-building banner + auto-reload when a permanent link is opened mid-build (July 5)
- Admin dashboard (admin.html): submission management, plan viewing, simulation retry, auto-polling (5s while active), interest list, student first+last names
...
- Final review + one fix attempt; honest completion status (completed_with_issues + alert email to Josie when a plan ships despite a failing final review)
- Regen fast-path: validator regen-severity flags skip the doomed fix loop
- All steps re-check Redis status for cancellation after Claude API call returns
- Tab-close survival + resume VERIFIED in a real browser (July 5): generation calls finish server-side after the tab dies; reopening intake.html resumes the pipeline; plan email arrives either way
...
- [x] **Re-baseline:** DONE July 4, 2026. Four profiles PASS (Brett, Sofia, Priya, Jake; 3/0/1 fix cycles, zero regens — beats the old Opus baseline of 5 attempts). The bug hunt along the way killed 15+ real bugs. Measured cost band ~$6-9/plan. See Test Results and data/TODO-session-status.md.
...
#### 3. Testing
- [x] **Website + intake form copy pass:** DONE July 4, 2026 (index.html + intake.html; honest generating-time expectations, thinking-pause status messages, live sample links, stale static samples deleted).
- [x] **UptimeRobot on /api/keep-alive:** DONE July 4, 2026.
- [x] **CRO's daughter test prep:** browser end-to-end test DONE July 5 (Imani Washington: real form, tab closed mid-build, resume + emails verified; 4 bugs found and fixed — resume crash, hidden banner, dead-looking poll, location shadowing). Remaining prep is non-technical: Josie's deep reads + the intro message to Jon (drafted July 5, sending day one at Datacor).
...
- [x] Run Washington family test (Atlanta GA, $72K) — DONE July 5, 2026 as the Imani Washington browser E2E run: Zell Miller/HOPE thresholds correct (including the GSFC core-GPA nuance), UGA in-state costs verified, no fabricated scholarships, honest "UGA has no BSN" and no-direct-admit-nursing answers. Doubled as the 12th-grade age-band test.
...
- [x] Custom domain (scaffoldcollegestrategy.com) for plan URLs — DONE. DNS stays at Squarespace (A @ → Vercel, CNAME www → Vercel); www verified with valid cert + 308 redirect to apex (July 4, 2026). Optional post-demo: update apex A record to 216.150.1.1 (Vercel's newer recommendation; old IP still works).
...
- [ ] **Server-side orchestration — FIRST post-demo project, before Stripe (sequencing decided July 5).** Each pipeline step already runs server-side and survives tab close (proven by the July 5 Imani test: Tier 1 finished writing after the tab died), but the step-to-step conductor is browser JS; closing the tab parks the pipeline until resume picks it up. Move the conductor server-side — Upstash QStash chain (each step enqueues the next) or a Vercel cron sweeper that advances parked submissions — so the browser becomes a pure status viewer and plans finish + email no matter what the family does. run-pipeline.js already contains the conductor logic to port. This also fully solves mobile (locked phones suspend tabs) and retires the intake form's "computer is comfortable" note. Rationale for before-Stripe: "customer paid, closed laptop, got nothing until they revisit" must never attach to a charge. ~1 day of work + test-run budget.
...
**Cost optimization**
- [ ] **Cheaper model for mechanical steps (unvalidated possibility — may not be worth it).** Generation stays on Fable 5; fix-plan (guided find/replace) and possibly reconcile are candidates for Haiku/Sonnet at a fraction of the cost — they're ~half the calls in a fix-heavy run. Only pursue if prompt caching alone doesn't get costs where they need to be; test against the five known-good July plans first. Keep review on Fable regardless (reviewer quality is load-bearing; see the grounding bugs of July 2).
...
- [ ] **Model upgrade playbook (not just deprecation-driven).** Two triggers: (a) forced — Anthropic retires the current model (lived it: claude-opus-4 deprecated June 2026, forced the Fable migration), and (b) chosen — a better/cheaper successor ships and Scaffold should adopt it proactively, since plan quality IS the product. Either way the June 2026 migration is the procedure: (1) read the new model's API notes for shape changes (params, thinking, prefills — Fable broke all three), (2) swap the model name in src/lib/config.js (single source of truth), (3) re-tune prompts if instruction-following changed, (4) full re-baseline on the standard test profiles (Brett/Sofia/Priya/Jake + Imani) and compare against the known-good July 2026 plans before trusting output, (5) update unit economics for the new pricing. Budget real time: the Fable migration surfaced 15+ bugs. Check Anthropic's model page + deprecation notices quarterly.
...
**Fable 5 re-baseline (July 4, 2026):** four profiles PASS — Brett (3 fixes, after the validator/data bug chain was fixed), Sofia (end-to-end in one submission), Priya (cleanest run ever, zero T1 fixes), Jake (one fix). Zero regenerations. Beats the February Opus baseline below. Plan IDs and details in CLAUDE.md Test Results.

**Browser end-to-end test (July 5, 2026):** Imani Washington (Atlanta GA, $72K, 12th-grade rising senior, nursing) PASS in ~22 min via the real intake form with a mid-build tab close. Verified the full customer journey (form → disconnect → resume → both emails → plan) and passed all three designed content traps (Zell Miller qualification at the threshold, no-BSN-at-UGA honesty, Emory need-based aid math). Found and fixed 4 render/resume bugs. Details in CLAUDE.md Test Results.

**February 2026 baseline (old Opus, for comparison):**
