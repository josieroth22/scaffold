...
7. **Budget alignment:** Does the family's stated budget appear prominently in your financial analysis? For EVERY school, did you compare the estimated net cost to the budget? Are over-budget schools clearly flagged in the executive summary table (e.g., "Over Budget" in the financial risk column)? CRITICAL RULE: **At least 60% of schools on the list must be at or under the family's stated budget.** If the family explicitly asked for expensive reach schools (Ivies, Stanford, etc.), you may include up to 3 over-budget reaches with clear "Over Budget" warnings, but the remaining 7+ schools MUST be at or under budget. If you have more than 4 over-budget schools, remove the least compelling over-budget school and replace it with an affordable option. Every list must include at least 3 schools that are confidently UNDER budget without relying on merit aid.
8. **JSON block consistency:** Will the JSON simulation params you output match the numbers in your narrative and table? For EACH school, verify: (a) **admit_pct uses exactly 3 decimal places matching your narrative percentage** (7.7% must be 0.077, 4.6% must be 0.046, 37.8% must be 0.378 -- if you rounded to 2 decimals like 0.08 or 0.05, FIX IT NOW), (b) sticker_cost minus midpoint of need aid range minus (merit_pct * midpoint of merit range) is within $3K of your narrative net cost estimate. If not, fix it.
...
10. **Fabrication check:** For EACH school on your list, go to the VERIFIED SCHOOL DATA and compare every scholarship name, honors program, and specific program you mentioned against what's listed there. If you named something that is NOT in the verified data for that school, delete the name and replace with a generic description. This includes partial matches -- if you added a scholarship name that's similar but not identical to one in the data, use the exact name from the data.
